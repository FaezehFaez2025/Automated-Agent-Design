...
AFlow's optimizer is built on a variant of **Monte Carlo Tree Search (MCTS)**, a heuristic search algorithm for decision-making that combines tree search with random sampling.

### Core Idea

Instead of exhaustively evaluating every possible workflow, MCTS runs many simulations to estimate which directions are most promising, then focuses exploration there.

### The Four Phases

Each iteration repeats four steps:

1. **Selection** — Traverse the tree from the root by picking children according to a selection policy (commonly **UCB1**), which balances *exploitation* (known good nodes) with *exploration* (less-visited ones), until reaching an unexpanded node.
2. **Expansion** — Add one or more child nodes representing new, unexplored states (in AFlow: new candidate workflows).
3. **Simulation (Rollout)** — From the new node, evaluate performance (in AFlow: run the workflow on validation samples and score it).
4. **Backpropagation** — Propagate the result back up the tree, updating each ancestor's visit count and score statistics.

After many iterations, the branch with the highest visit count or score from the root is selected.

### Why It Works

- **No hand-crafted evaluation function needed** — rollouts provide a statistical estimate of value.
- **Asymmetric tree growth** — computation naturally focuses on promising branches.
- **Anytime algorithm** — can be stopped at any point and returns the best result found so far.

### Famous Applications

- **Go** — AlphaGo (DeepMind) combined MCTS with neural networks to defeat world champions.
- **Chess & Shogi** — AlphaZero extended the approach across multiple games.
- **General planning** — Robotics, scheduling, and combinatorial optimization.

### Key Tradeoff

MCTS trades exactness for scalability. It won't always find the optimal solution, but finds very good solutions efficiently in state spaces too large for exhaustive search — making it well-suited for AFlow's vast workflow search space.
